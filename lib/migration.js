            ...
            async.each(triggersArr, function(triggerObj, asyncCB) {
              ...
              Model.dataSource.connector.query(modelQuery, [], triggerObj.queryContext, function (err, result) {
                if (err || result.length === 0) {
                  ...
                  Model.dataSource.connector.query(modelQuery, [], triggerObj.queryContext, function (err, result) {
                    if (err) {
                      console.log('could not insert trigger function on model');
                      return asyncCB(new Error('could not insert trigger function on model'));
                    }
                    return asyncCB();
                  });
                } else if (result.length > 1) {
                  console.log('too many triggers');
